...
from ..net_utils import IlluminationGuidedFeatureRectifyModule as IG_FRM
import math
import time
...
class RGBXTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512], 
        ...
        self.FRMs = nn.ModuleList([
            IG_FRM(dim=embed_dims[0], reduction=1),
            IG_FRM(dim=embed_dims[1], reduction=1),
            IG_FRM(dim=embed_dims[2], reduction=1),
    ...
    def forward_features(self, x_rgb, x_e):
        ...
        x_gray = x_rgb.mean(dim=1)
        def gaussian(x, mu=0.5, sigma=0.15):
            return torch.exp(-((x - mu) ** 2) / (2 * sigma ** 2))
        score_map = gaussian(x_gray)
        ...
        B,C,H,W = x_rgb.size()
        x_score = F.interpolate(score_map,[H,W],mode="nearest").expand_as(x_rgb)
        x_rgb, x_e = self.FRMs[0](x_score, x_rgb, x_e)
        ...
        B,C,H,W = x_rgb.size()
        x_score = F.interpolate(score_map,[H,W],mode="nearest").expand_as(x_rgb)
        x_rgb, x_e = self.FRMs[1](x_score, x_rgb, x_e)
        ...
        B,C,H,W = x_rgb.size()
        x_score = F.interpolate(score_map,[H,W],mode="nearest").expand_as(x_rgb)
        x_rgb, x_e = self.FRMs[2](x_score, x_rgb, x_e)
